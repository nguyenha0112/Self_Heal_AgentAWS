...
|---|---|---|
| Service stuck / latency spike | `RESTART_DEPLOYMENT` | Khớp telemetry `service_latency_p95` và action trong AI API Contract |
| Error rate spike / code-level fault | `RESTART_DEPLOYMENT` hoặc escalate với context bundle | Khớp `service_error_rate`, `application_log_event`, `distributed_trace_error_event` |
| Memory pressure / OOM prevention | `PATCH_MEMORY_LIMIT` hoặc escalate nếu không đủ an toàn | Khớp telemetry `container_resource_usage` |
...
|---|---|---|
| Queue/backpressure | `SCALE_REPLICAS` | Action có trong AI contract mới, nhưng cần dữ liệu queue rõ hơn |
| Secret/cert/config issue | `ROTATE_SECRET` | Action có trong AI contract mới, nhưng rủi ro cao hơn nên để design-only nếu chưa đủ guardrail |
...
| `container_resource_usage` | Đọc memory usage theo container/pod | Detect memory pressure/OOM prevention |
...
- Với CDO-02, tenant ID tạm map theo deployment contract mới là `6c8b4b2b-4d45-4209-a1b4-4b532d56a31c`; cần AI xác nhận đây là ID chính thức.
...
- Auth: IAM SigV4.
- Tenant ID cho CDO-02: `6c8b4b2b-4d45-4209-a1b4-4b532d56a31c` (pending AI confirmation).
...
4. Offline Simulation Mode đã là Mock Mode theo contract; AI/trainer có yêu cầu thêm action thật trên Kubernetes sandbox để demo không?
5. AI skeleton endpoint khi nào có để CDO test integration?
6. Với `503`, CDO nên ưu tiên static runbook fallback hay escalation thẳng cho SRE?
7. Nếu AI contract mới không dùng SQS, CDO có được tự giữ SQS như buffer nội bộ không?
